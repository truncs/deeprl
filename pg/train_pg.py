...
def train_PG(exp_name='',
    ...
    if discrete:
        ...
        sy_logstd = tf.Variable(tf.zeros([1, ac_dim]), name='sy_logstd', dtype=tf.float32)
        sy_std = tf.exp(sy_logstd)

        sy_sampled_ac = tf.random_normal(tf.shape(sy_mean), mean=sy_mean, stddev=sy_std)
    ...
    if nn_baseline:
        ...
        # YOUR_CODE_HERE
        baseline_target = tf.placeholder(shape=[None], name='baseline_target', dtype=tf.float32)
        baseline_loss = tf.reduce_sum(tf.losses.mean_squared_error(
            labels=baseline_target,
    ...
    for itr in range(n_iter):
        ...
        if nn_baseline:
            ...
            sess.run(
                [baseline_update_op],
                feed_dict={sy_ob_no: ob_no, baseline_target: q_n}
            )
            pass
